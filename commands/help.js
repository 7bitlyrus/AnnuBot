...
class Help extends Command {
  constructor () {
    ...
    this.usage.text = 'If command is not specified, returns a list of commands. Otherwise, shows usage for the given' +
      ' command.'
  }
  ...
  async execute (msg, args) {
    ...
    if (!args[0]) {
      ...
      for (let [key, command] of commands) {
        if (key !== command.constructor.name.toLowerCase()) continue
